    ...
    <div>
      <svg class="graph" width="300" height="300">
        ...
        {#each graph.vertices as vertex, index}
          <circle class={"vertex" + index} cx={vertex.x} cy={vertex.y} r="16"
          ></circle>
          <text dominant-baseline="middle" x={vertex.x} y={vertex.y}>{index}</text
          >
        {/each}
      </svg>
    </div>
    <div>
      <svg width="400" height="500">
        ...
        {#each decomposition.nodes as node, index}
          <rect 
          class:badnode={decomposition.bags[index].length-1 > expectedTreeWidth}
          rx="16" x={node.x - 48} y={node.y - 48} width="96" height="96"
          ></rect>
          {#each graph.vertices as vertex, vertexi}
            <circle
              on:click={() => toggleVertex(index, vertexi)}
              class:bad={!isConnected(decomposition, vertexi) &&
                decomposition.bags[index].some((v) => v.i == vertexi)}
